// test_fd_edge_cases.c - Tests pour les file descriptors
#include <stdio.h>
#include <stdlib.h>
#include <fcntl.h>
#include <unistd.h>
...
int main(void)
{
    // ========== Tests avec FD invalides (ne doivent pas crash) ==========
    
    // Test 1: FD négatif (ne doit pas crash)
    ft_putchar_fd('A', -1);
    ...
    // Test 2: FD très grand (probablement invalide)
    ...
    if (fd >= 0) {
        close(fd);
        // Écrire sur un FD fermé ne doit pas crash
        ft_putchar_fd('A', fd);
        TEST("putchar_fd on closed fd doesn't crash", 1);
    }
    
    // ========== Tests avec NULL (ne doivent pas crash) ==========
    
    ft_putstr_fd(NULL, 1);
    TEST("putstr_fd with NULL doesn't crash", 1);
    
    ft_putendl_fd(NULL, 1);
    TEST("putendl_fd with NULL doesn't crash", 1);
    
    // ========== Tests fonctionnels avec fichier temporaire ==========
